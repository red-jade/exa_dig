defmodule Exa.Dig.BuildTest do
  use ExUnit.Case
  ...
  @dig_out_dir Path.join(["test", "output", "dig"])

  test "dig dust" do
    {h1in, h1out, h1inout, h2inout} = digger("dust", &Build.dust/1)
    assert h1in == [10]
    assert h1out == [10]
    assert h1inout == [10]
  ...
  test "dig line   " do
    {h1in, h1out, h1inout, h2inout} = digger("line", &Build.line/1)
    assert h1in == [1, 9]
    assert h1out == [1, 9]
  ...
  test "dig ring   " do
    {h1in, h1out, h1inout, h2inout} = digger("ring", &Build.ring/1)
    assert h1in == [0, 10]
    assert h1out == [0, 10]
  ...
  test "dig fan_in " do
    {h1in, h1out, h1inout, h2inout} = digger("fan_in", &Build.fan_in/1)
  ...
  test "dig fan_out" do
    {h1in, h1out, h1inout, h2inout} = digger("fan_out", &Build.fan_out/1)
    assert h1in == [1, 9]
  ...
  test "dig wheel  " do
    {h1in, h1out, h1inout, h2inout} = digger("wheel", &Build.wheel/1)
    assert h1in == [1, 0, 9]
  ...
  test "dig clique " do
    {h1in, h1out, h1inout, h2inout} = digger("clique", &Build.clique/1)
  ...
  test "dig grid2d" do
    {h1in, h1out, h1inout, h2inout} = Build.grid2d(4, 3) |> render("grid2d")
  ...
  defp render(g, title) do
    ...
    Enum.each([:png, :svg], fn fmt ->
      Render.render_dot(dotfile, fmt, @dig_out_dir)
    end)
