defmodule Exa.Dig.DigTest do
  ...
  @in_dir ["test", "input", "dig"]

  defp dot_file(name), do: Exa.File.join(@in_dir, name, @filetype_dot)
  ...
  test "edge create vertex" do
    g = Dig.new()
    {8, 9} = e = Dig.add_edge(g, 8, 9, true)
    assert 2 == Dig.nvert(g)
    assert 1 == Dig.nedge(g)
    assert [e] == Dig.edges(g)

    assert {:error, "Missing vertex 4"} == Dig.add_edge(g, 4, 5, false)
  ...
  test "repeat edge" do
    g = Dig.new()
    {8, 9} = Dig.add_edge(g, 8, 9)
    assert {:error, "Existing edge {8,9}"} == Dig.add_edge(g, 8, 9, false)
    Dig.delete(g)
  end

  test "self loop and cycles" do
    g = Dig.new(:cyclic)
    {8, 8} = Dig.add_edge(g, 8, 8)
    Dig.delete(g)

    g = Dig.new(:acyclic)
    assert {:error, "Cyclic path [8, 8]"} == Dig.add_edge(g, 8, 8)
    Dig.add_edge(g, 4, 5)
    assert {:error, "Cyclic path [4, 5]"} == Dig.add_edge(g, 5, 4)
    Dig.delete(g)
  end

  test "delete edge" do
    g = Dig.new()
    {8, 9} = Dig.add_edge(g, 8, 9, true)
    assert 2 == Dig.nvert(g)
    assert 1 == Dig.nedge(g)
    Dig.del_edge(g, {8, 9})
    assert 2 == Dig.nvert(g)
    assert 0 == Dig.nedge(g)
    assert {:error, "Missing edge {8,9}"} == Dig.del_edge(g, {8, 9})
  end
  ...
  test "delete vertex" do
    ...
    assert {:error, "Missing vertex 13"} == Dig.del_vert(g, 13)
    Dig.delete(g)
  end
